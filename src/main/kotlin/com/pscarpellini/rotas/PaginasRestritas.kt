...
fun Route.paginasRestritas(
    ...
    post(PaginasRestritasEnum.EXIBIR_CONFIGURACOES_DO_APP.caminho.path) {
        handleExibirConfiguracoesDoApp(
            clienteRepository
        )
    }
    post(PaginasRestritasEnum.EDITAR_CONFIGURACOES_DO_APP.caminho.path) {
        handleEditarConfiguracoesDoApp(
            clienteRepository
        )
    }
    post(CaminhosBaseEnum.FORMULARIO_EDITAR_CONFIGURACOES_APP.path) {
        handleFormularioEditarConfiguracoesApp(
            clienteRepository
        )
    }
    ...
    post(CaminhosComissaoEnum.FORMULARIO_HABILITAR_VIDEO.path) {
        handleHabilitarDesabilitarVideo(
            videosRepository,
            habilitar = true
        )
    }
    post(CaminhosComissaoEnum.FORMULARIO_DESABILITAR_VIDEO.path) {
        handleHabilitarDesabilitarVideo(
            videosRepository,
            habilitar = false
        )
    ...
    post(PaginasRestritasEnum.FORMULARIO_EDITAR_USUARIO.caminho.path) {
        handleFormularioEditarUsuario(contasRepository)
    }
    ...
    post(CaminhosBaseEnum.FORMULARIO_ALTERAR_SENHA_PERFIL.path) {
        handleFormularioMeuPerfilAlterarSenha(
            contasRepository,
            emailSender
        )
    }
